...
|---|---|---|
| `/robots.txt` | 404 | 200, allows `/`, disallows the 15 proxied prefixes + `/api/`, names the sitemap |
| `/sitemap.xml` | 404 | 200, lists `/` and `/login` as absolute www URLs |
| `<link rel="canonical">` | absent everywhere | self-referencing on every route |
| `metadataBase` | `https://shaipt.com` (the redirecting apex) | `https://www.shaipt.com` |
| `<h1>` on the landing page | three | one |
| JSON-LD | none | 3 blocks: `SoftwareApplication`, `Organization`, `FAQPage` |
| OG image | `logo_transparent.png`, 512×512 in a 1.91:1 slot | `og.png`, 1200×630 |
| `/demo` (6 pages) | indexable, thin, duplicate | `noindex, follow` |
| Tests | no SEO coverage | 30 Jest + 7 Playwright |

Unchanged and still true: `https://www.shaipt.com/` is 200 and server-rendered (~6,000 characters
of real text); `https://shaipt.com` 307s to www; the site is indexed for "SHaiPT" today; the
landing copy is still almost entirely about 4D form checking rather than "AI personal trainer".

Notes for whoever picks this up:

- **`alternates.canonical` is `"./"`, not `"/"`.** The original plan in this file said `"/"`. That
  is wrong: metadata set in the root layout is inherited by child routes as-is, so `"/"` makes
  `/login` declare itself a duplicate of the landing page. `"./"` resolves against the current
  route. Verified in the build output — `/login` emits `https://www.shaipt.com/login`.
- **`/demo` is `noindex` but deliberately NOT disallowed in robots.txt.** A crawler has to be
  allowed to fetch a page before it can read the directive telling it not to index it.
  `app/demo/layout.tsx` was a client component and could not export metadata; the chrome moved to
  `components/demo/DemoChrome.tsx` and the layout is now a server component.
- **`app/page.tsx` is now a server component** that emits the JSON-LD and renders
  `components/landing/LandingPage.tsx` (the old page body, unchanged, still `'use client'`).
- **The FAQ items and pricing tiers live in `lib/landing/`** and are read by both the visible
  components and the JSON-LD, so the markup cannot drift from the page. Do not re-inline them.
- `public/og.png` is generated by `node scripts/generate-og.mjs` (Playwright screenshot of
  brand-styled HTML). Re-run it if the card copy changes.
...
tell "not indexed yet" apart from "not working".

| Date | Property | Action |
|---|---|---|
...
Kept for the record — this is the state the work started from.
...
## Workstream A — the technical floor — DONE 2026-09-09, see the status log above
