...
export default function AnalyticsDashboardPage() {
    ...
    /** The shape WeeklyInsightsCard reads: one entry per workout, its exercises named. */
    const insightLogs = useMemo(
        () =>
            workoutLogs.map((log) => ({
                id: log.id,
                date: log.date,
                exercises: (log.exercise_logs || []).map((el) => ({
                    name: exercises[el.exercise_id]?.name,
    ...
    return (
        <div className="mx-auto min-h-screen max-w-[900px] p-6 pb-24">
            ...
            {/* This week, read by the coach: one cached model call a week (see /api/ai-coach/weekly-insights) */}
            {workoutLogs.length > 0 && (
                <div className="mb-8">
                    <WeeklyInsightsCard logs={insightLogs} />
                </div>
            )}
